...
const roles = ["Software Engineer", "Product Manager", "Data Scientist", "UX Designer", "Engineering Manager"];
const difficulties = ["Entry Level", "Mid Level", "Senior", "Staff / Principal"];
const types = [
    ...
    <div className="h-full flex flex-col md:flex-row gap-5 animate-fade-in">
      ...
      <aside className="md:w-72 shrink-0 space-y-4">
        ...
        <div className="glass rounded-2xl p-4 border border-slate-700/40">
          <h3 className="font-display font-semibold text-white text-sm mb-3">Session Setup</h3>
          <div className="space-y-3">
            <div>
              <label className="text-xs text-slate-500 font-body block mb-1.5">Target Role</label>
            ...
            <div>
              <label className="text-xs text-slate-500 font-body block mb-1.5">Difficulty</label>
              <div className="relative">
                <select
                  value={difficulty}
        ...
        {/* Interview type */}
        <div className="glass rounded-2xl p-4 border border-slate-700/40">
          <label className="text-xs text-slate-500 font-body block mb-3">Interview Type</label>
          <div className="grid grid-cols-2 gap-2">
            {types.map((t) => (
              <button
                key={t.label}
                onClick={() => setInterviewType(t.label)}
                className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border text-xs font-body font-medium transition-all duration-150 ${
              ...
              >
                <t.icon size={14} className={interviewType === t.label ? "text-indigo-400" : t.color} />
                {t.label}
              </button>
            ))}
          </div>
        ...
        <div className="glass rounded-2xl p-4 border border-slate-700/40 space-y-3">
          {started && (
            <div className="flex items-center gap-2 bg-slate-800 rounded-xl px-4 py-2.5">
              <Timer size={15} className="text-cyan-400 animate-pulse" />
              <span className="font-mono text-cyan-300 text-sm font-medium">{formatTime(elapsed)}</span>
              <span className="text-xs text-slate-500 ml-auto">elapsed</span>
            </div>
          )}
          {!started ? (
            <Button className="w-full justify-center" icon={Play} onClick={handleStart}>
              Start Session
            </Button>
          ) : (
            <Button
              ...
              icon={RotateCcw}
              onClick={() => { setStarted(false); setElapsed(0); }}
            >
              New Session
            </Button>
          )}
          <Button variant="ghost" size="sm" className="w-full justify-center" icon={Settings2}>
            Advanced Settings
          </Button>
        </div>
        ...
        <div className="glass rounded-2xl p-4 border border-cyan-500/15">
          ...
          <p className="text-xs text-slate-400 leading-relaxed font-body">
            Use the <strong className="text-slate-300">STAR method</strong> — Situation, Task, Action, Result — to structure behavioral answers for maximum impact.
          </p>
        </div>
      </aside>
        ...
        {started ? (
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center max-w-xs">
              ...
              <p className="font-display font-semibold text-white mb-2">Ready when you are</p>
              <p className="text-sm text-slate-500 font-body leading-relaxed">
                Configure your session on the left and hit <strong className="text-slate-400">Start Session</strong> to begin your mock interview.
              </p>
            </div>
          </div>
        )}
